...
namespace X42.Utilities
{
    ...
    ///         to make the task run only once. Running the task for other than one or infinite number of times is not
    ///         supported.
    ///     </para>
    /// </summary>
    public class AsyncLoop : IAsyncLoop
    {
        ...
        public AsyncLoop(string name, ILogger logger, Func<CancellationToken, Task> loop)
        {
            ...
            this.logger = logger;
            loopAsync = loop;
            RepeatEvery = TimeSpan.FromMilliseconds(1000);
        }
        ...
        public IAsyncLoop Run(CancellationToken cancellation, TimeSpan? repeatEvery = null, TimeSpan? startAfter = null)
        {
            ...
            RunningTask = StartAsync(cancellation, startAfter);
        ...
        private Task StartAsync(CancellationToken cancellation, TimeSpan? delayStart = null)
        {
            return Task.Run(async () =>
            {
                ...
                try
                {
                    if (delayStart != null)
                    {
                        logger.LogInformation("{0} starting in {1} seconds.", Name, delayStart.Value.TotalSeconds);
                        await Task.Delay(delayStart.Value, cancellation).ConfigureAwait(false);
                    }

                    logger.LogInformation("{0} starting.", Name);

                    if (RepeatEvery == TimeSpans.RunOnce)
                    {
                    ...
                    while (!cancellation.IsCancellationRequested)
                    {
                        await loopAsync(cancellation).ConfigureAwait(false);
                        if (!cancellation.IsCancellationRequested)
                            await Task.Delay(RepeatEvery, cancellation).ConfigureAwait(false);
                    }
                }
                ...
                {
                    logger.LogInformation(Name + " stopping.");
                }
                ...
                if (uncaughtException != null)
                {
                    ...
                    logger.LogCritical(new EventId(0), uncaughtException, Name + " threw an unhandled exception");

                    // You can touch this one.
                    logger.LogError("{0} threw an unhandled exception: {1}", Name, uncaughtException.ToString());
                }
            }, cancellation);
        }
    }
