...
namespace X42.Utilities
{
    ...
    public interface IAsyncLoopFactory
    {
        ...
        ///     Interval between each execution of the task.
        ///     If this is <see cref="TimeSpans.RunOnce" />, the task is only run once and there is no loop.
        ///     If this is null, the task is repeated every 1 second by default.
        /// </param>
        /// <param name="startAfter">Delay before the first run of the task, or null if no startup delay is required.</param>
        IAsyncLoop Run(string name, Func<CancellationToken, Task> loop, TimeSpan? repeatEvery = null,
        ...
        /// </param>
        /// <param name="startAfter">Delay before the first run of the task, or null if no startup delay is required.</param>
        IAsyncLoop Run(string name, Func<CancellationToken, Task> loop, CancellationToken cancellation,
            TimeSpan? repeatEvery = null, TimeSpan? startAfter = null);
        ...
        /// </summary>
        /// <param name="name">Name of the loop.</param>
        /// <param name="serverCancellationToken">Cancellation token that triggers when the task and the loop should be cancelled.</param>
        /// <param name="condition">Condition to be tested.</param>
        /// <param name="action">Method to execute once the condition is met.</param>
        /// <param name="onException">
        ///     Method to execute if an exception occurs during evaluation of the condition or during
        ///     execution of the <see cref="action" />.
        /// </param>
        /// <param name="repeatEvery">
        ///     Interval between each execution of the task.
        ///     If this is <see cref="TimeSpans.RunOnce" />, the task is only run once and there is no loop.
        ...
        /// </param>
        /// <returns></returns>
        IAsyncLoop RunUntil(string name, CancellationToken serverCancellationToken, Func<bool> condition, Action action,
    ...
    public sealed class AsyncLoopFactory : IAsyncLoopFactory
    {
        ...
        /// <remarks>
        ///     TODO: It might be a better idea to pass factory to the newly created loops so that new loggers can be created
        ///     for each loop.
        /// </remarks>
        ...
        {
            return new AsyncLoop(name, logger, loop);
        }

        /// <inheritdoc />
        public IAsyncLoop Run(string name, Func<CancellationToken, Task> loop, TimeSpan? repeatEvery = null,
            TimeSpan? startAfter = null)
        {
        ...
        /// <inheritdoc />
        public IAsyncLoop Run(string name, Func<CancellationToken, Task> loop, CancellationToken cancellation,
            TimeSpan? repeatEvery = null, TimeSpan? startAfter = null)
        {
            ...
            return new AsyncLoop(name, logger, loop).Run(cancellation, repeatEvery ?? TimeSpan.FromMilliseconds(1000),
                startAfter);
        }
        ...
        {
            var linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(serverCancellationToken);
