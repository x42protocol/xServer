...
namespace X42.Utilities
{
    ...
    public interface IAsyncLoopFactory
    {
        ...
        /// If this is null, the task is repeated every 1 second by default.</param>
        /// <returns></returns>
        IAsyncLoop RunUntil(string name, CancellationToken serverCancellationToken, Func<bool> condition, Action action, Action<Exception> onException, TimeSpan repeatEvery);
    }
    ...
    public sealed class AsyncLoopFactory : IAsyncLoopFactory
    {
        ...
        {
            this.logger = loggerFactory.CreateLogger(typeof(X42Server).FullName);
        }
        ...
        /// <inheritdoc />
        public IAsyncLoop RunUntil(string name, CancellationToken serverCancellationToken, Func<bool> condition, Action action, Action<Exception> onException, TimeSpan repeatEvery)
        {
