...
var mainWindow = null;
var daemonState;
var xServerDaemonState;
var contents = null;
var currentNetwork;
var settings;
var args = process.argv.slice(1);
var serve = args.some(function (val) { return val === '--serve'; });
var coin = { identity: 'x42', tooltip: 'xServer' };
...
electron_1.ipcMain.on('start-daemon', function (event, arg) {
    ...
    if (arg.mode === 'manual') {
        ...
        var msg = 'xServer was started in development mode. This requires the user to be running the daemon manually.';
        writeLog(msg);
...
electron_1.ipcMain.on('start-xserver-daemon', function (event, arg) {
    ...
    if (arg.mode === 'manual') {
        ...
        var msg = 'xServer was started in development mode. This requires the user to be running the daemon and xserver.d manually.';
        writeLog(msg);
        event.returnValue = msg;
    }
    else {
...
electron_1.ipcMain.on('reset-database', function (event, arg) {
    ...
    shutdownDaemon(function (success, error) {
        ...
        var dataFolder = path.join(appDataFolder, 'x42', arg);
        ...
        var txdb = path.join(dataFolder, 'txdb');
...
var shutdownXServer = function (callback) {
    shutdownXServerDaemon(function (success, error) {
        if (success) {
            ...
            writeError(error);
            // Perform another retry, and quit no matter the result.
            shutdownXServerDaemon(function (ok, err) {
                if (ok) {
...
function startXServerDaemon(chain) {
    var xServerfolderPath = chain.xServerPath || getxServerDaemonPath();
    var xServerDaemonName;
    if (chain.name === 'x42') {
    ...
    var xServerDaemonPath = path.resolve(xServerfolderPath, xServerDaemonName);
...
function launchxServerDaemon(apiPath, chain) {
    var daemonxServerProcess;
    // TODO: Consider a future improvement that would ensure we don't loose a reference to an existing spawned process.
    // If launch is called twice, it might spawn two processes and loose the reference to the first one, and the new process will die due to TCP port lock.
    var spawnxServerDaemon = require('child_process').spawn;
    var commandLineArguments = [];
    if (chain.mode === 'local') {
        if (!apiPath || apiPath.length < 3 || !chain.datafolder || chain.datafolder.length < 3) {
    ...
    daemonxServerProcess.stdout.on('data', function (data) {
    ...
    daemonxServerProcess.on('error', function (code, signal) {
        writeError("x42 Node process failed to start. Code " + code + " and signal " + signal + ".");
    });
}
